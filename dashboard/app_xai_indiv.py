...
def xai_indiv():
    ...
    charts = [
        histogram_chart(score_df[score_df[TARGET] == lb]).properties(
            title=f"Class = {lb}"
        )
    ...
    select_samples = x_sample.index[
        (y_sample == class_idx) & (scores_bin == bin_idx)]
    ...
    source = make_source_waterfall(
        instance, base_value, shap_values, max_display=15)
