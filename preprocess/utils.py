...
def get_execution_date():
    ...
    # Run DAG daily at 01H15 UTC = 09H15 SGT
    if not execution_date:
        dt_now = datetime.now(pytz.utc) - timedelta(days=1)
        # If time now is between 00H00 UTC and 01H15 UTC, set execution date 1 more day earlier
        if dt_now.strftime("%H:%M") < "01:15":
...
def onehot_enc(df, categorical_columns, categories):
    ...
    df1 = pd.DataFrame(y, columns=ohe_feats)
    
    output_df = pd.concat([df[noncategorical_cols], df1], axis=1)
    return output_df, ohe_feats
