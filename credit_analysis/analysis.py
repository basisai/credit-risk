...
def odds_chart(y_true, y_prob, y_baseline, num_bins):
    ...
    c = alt.Chart(source).mark_bar().encode(
        ...
        color='Model:N',
        column=alt.Column('Pred Proba of Default:N', title="Predicted Probability of Default Bin"),
...
def metrics_tables(y_true, y_prob, y_baseline, threshold):
    ...
    st.write(pd.DataFrame(
        ...
        index=["Mean default rate in predicted bad group",
...
def stats_table(y_true, y_prob, y_baseline, threshold):
    df1 = acct_table(y_true, y_prob, threshold, LOSS_PER_BAD_ACCT, REV_PER_GOOD_ACCT)
...
def compare_models():
    ...
    st.altair_chart(heatmap_chart(score_shift_matrix), use_container_width=False)
